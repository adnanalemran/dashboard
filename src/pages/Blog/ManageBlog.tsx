...
const ManageBlog = () => {
  ...
  const { data: newss = [], refetch } = useQuery({
    queryKey: ['news'],
    queryFn: async () => {
  ...
  const handleDeleteNoticce = (news) => {
    Swal.fire({
      title: 'Are you sure?',
      ...
      if (result.isConfirmed) {
        axiosSecure.delete(`/blog/${news?._id}`).then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    const { title, image, notice: news } = e.target.elements;

    const noticeTitle = title.value;
    const imageUrl = image.value;
    const noticeDetails = news.value;
  ...
  return (
    <div>
      <DefaultLayout>
        <Breadcrumb pageName="Manage Blog/ news " />
        ...
        <div className="text-black">
          <div className="card  rounded-sm border border-stroke bg-white p-4 shadow-default dark:border-strokedark dark:bg-boxdark md:p-6 xl:p-9   ">
            <div className=" mb-6   font-semibold text-black dark:text-white">
              <h4> Total Publish news : {newss?.length}</h4>
            </div>
                ...
                <div className="flex flex-col">
                  ...
                  {newss.map((news, key) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-center p-2.5 xl:p-5">
                        <p className="text-meta-3">{news.postDate}</p>
                      </div>
